...
def transform_dataset(
    ...
    if dataset.y is None or dataset.y.get('train') is None:
        y, y_info = None, None
    else:
        y, y_info = build_target(dataset.y, transformations.y_policy, dataset.task_type)
...
def prepare_fast_dataloader(
    ...
    if D.y is None or D.y.get(split) is None:
        # create an (N,0) float tensor so Trainer can still unpack (X,y)
        y = torch.empty((X.shape[0], 0), dtype=torch.float32, device=X.device)
    else:
