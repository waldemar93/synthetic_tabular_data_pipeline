...
def simple_sample(
    ...
    if D.y is not None and D.y.get('train') is not None:
        y_train = D.y['train']

    if y_train is not None:
        _, empirical_class_dist = torch.unique(torch.from_numpy(y_train), return_counts=True)
    else:
        empirical_class_dist = None

    if empirical_class_dist is not None and disbalance == 'fix':
        empirical_class_dist[0], empirical_class_dist[1] = empirical_class_dist[1], empirical_class_dist[0]
        x_gen, y_gen = diffusion.sample_all(num_samples, batch_size, empirical_class_dist.float(), ddim=False)

    elif empirical_class_dist is not None and disbalance == 'fill':
        ix_major = empirical_class_dist.argmax().item()
        val_major = empirical_class_dist[ix_major].item()
        x_gen, y_gen = [], []
        for i in range(empirical_class_dist.shape[0]):
        ...
        if empirical_class_dist is not None:
            x_gen, y_gen = diffusion.sample_all(num_samples, batch_size, empirical_class_dist.float(), ddim=False)
        else:
            x_gen, y_gen = diffusion.sample_all(num_samples, batch_size, None, ddim=False)

    if x_gen is None and y_gen is None:
    ...
    num_numerical_features = num_numerical_features_ + int(D.is_regression and model_params["is_y_cond"])
    ...
    if num_numerical_features_ != 0:
        ...
        # only strip off a “target” column when this was a supervised regression
        if model_params["is_y_cond"] and model_params['num_classes'] == 0:
            y_gen = X_num[:, 0]
            X_num = X_num[:, 1:]
    ...
    if num_numerical_features > 0:
        if num_numerical_features < X_gen.shape[1]:
            ...
            return X_num, None, y_gen
    else:
        return None, X_cat, y_gen


if __name__ == '__main__':
    raw_config = lib.load_config('data/adult/config.toml')
    '''
    sample(
    ...
    '''
    x_num = np.load('X_num_train.npy')
    x_cat = np.load('X_cat_unnorm.npy')
