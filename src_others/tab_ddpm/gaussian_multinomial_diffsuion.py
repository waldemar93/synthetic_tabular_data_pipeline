...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def q_posterior(self, log_x_start, log_x_t, t):
        ...
        log_EV_xtmin_given_xt_given_xstart = \
            unnormed_logprobs \
            - sliced_logsumexp(unnormed_logprobs, self.offsets.long())
    ...
    def mixed_loss(self, x, out_dict):
        ...
        if 'y' in out_dict:
            model_out = self._denoise_fn(x_in, t, y=out_dict['y'])
        else:
        ...
        if x_cat.shape[1] > 0:
            loss_multi = self._multinomial_loss(model_out_cat, log_x_cat, log_x_cat_t, t, pt, out_dict)
            loss_multi = loss_multi / max(1, len(self.num_classes))
    ...
    def sample_ddim(self, num_samples, y_dist):
        ...
        if y_dist is None:
            # dummy empty label vector of shape (b,0)
            y = torch.empty((b, 0), dtype=torch.long, device=device)
        else:
            y = torch.multinomial(y_dist, num_samples=b, replacement=True).long().to(device)
        out_dict = {'y': y}
