# Synthetic Clinical Data Pipeline

Code accompanying the PhD thesis *Model-Agnostic Quality Improvement for Synthetic Tabular Clinical Data Generation and Publication*.

The repository implements preprocessing, hyperparameter optimization, model training, synthetic-data evaluation, and privacy evaluation.

## Getting started

Clone this repository from GitHub and create a virtual environment and activate it. 
Install the requirements with `pip install -r requirements.txt`.
The complete pinned environment was tested with **Python 3.10.20** on Linux and includes the dependencies for
CTGAN, TVAE, CTAB-GAN+, TAB-DDPM, and the SynthCity models.
...
`optimization_metric{number}_params`. 
...
`metric{number}_params`. **{number}** depends on the position in the `metrics` list (starting with 1).
...
Privacy evaluation is intended for selected synthetic datasets. Create a config file in
`config/privacy_eval_gen/` similar to `synthetic_aml_best.yaml` and select one or more entries in `privacy_metrics`:

- `hamming_distance`: the binned Hamming-distance evaluation.
- `authenticity`: Gower-distance authenticity, including too-close and near-duplicate summaries.
- `nearest_neighbor_adversarial_accuracy`: train/test nearest-neighbor adversarial accuracy and privacy loss.

For the two Gower-based metrics, numeric ranges are derived from the real training split. Categorical and boolean
variables are taken from the experiment's `columns.json`. `authenticity_epsilon` controls the near-duplicate threshold,
and `gower_block_size` controls the number of source rows processed at once without changing the formula.
...
filename (without the **.yaml** ending) and run `python -m src.privacy_evaluation`.
