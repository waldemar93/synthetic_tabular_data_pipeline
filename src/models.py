...
from src.helpers import get_categorical_columns_from_df
...
available_models = ['nflow', 'tvae', 'rtvae', 'bayesian_network', 'survae', 'survival_ctgan', 'ctgan', 'survival_nflow',
                    'survival_gan', 'ctab-gan+', 'tab_ddpm', 'ddpm']
...
'survival_nflow', 'survival_gan', 'ctab-gan+', 'tab_ddpm', 'ddpm']
available_synthcity_models_type = Literal['nflow', 'rtvae', 'bayesian_network', 'survae', 'survival_ctgan',
'survival_nflow', 'survival_gan', 'ddpm']

## monkey patch for ddpm
_np_append_orig = np.append


# Monkey‑patch np.append to handle torch.Tensor
def _safe_append(arr, values, axis=None):
    # if either argument is a CUDA tensor, detach→cpu→numpy
...
class Ctgan(GenerativeModel):
    ...
    def load_model(experiment_name: str, filename: str) -> GenerativeModel:
        ...
        loaded_ctgan.model = CTGAN.load(
            os.path.join(PROJECT_ROOT, 'experiments', experiment_name,
...
class CtabGanPlus(GenerativeModel):
    ...
    def load_columns_info(experiment_name: str):
        ...
        path = os.path.join(PROJECT_ROOT, 'experiments', experiment_name,
    ...
    def load_model(experiment_name: str, filename: str) -> GenerativeModel:
        ...
        if not filename.endswith('.pkl'):
            filename += '.pkl'
        filehandler = open(os.path.join(PROJECT_ROOT, 'experiments', experiment_name,
    ...
    def save_model(self, experiment_name: str, filename: str) -> None:
        ...
        file = open(os.path.join(PROJECT_ROOT, 'experiments', experiment_name,
...
class SynthCityModel(GenerativeModel):
    class CustomUnpickler(pickle.Unpickler):
        ...
        def find_class(self, module, name):
            ...
            if module == 'pathlib' and name in {'WindowsPath'}:
                return Path
    ...
    def save_model(self, experiment_name: str, filename: str) -> None:
        ...
        filename = filename if filename.endswith('.pkl') else filename + '.pkl'
        save_to_file(os.path.join(PROJECT_ROOT, 'experiments', experiment_name,
...
class Tab_ddpm(GenerativeModel):
    ...
    def __init__(self, model_type: Literal['resnet', 'mlp'], model_params: dict,
                 gaussian_loss_type: Literal['kl', 'mse'], df: DataFrame, T: dict, info_json: dict,
                 pred_column: str | None, num_timesteps: int, batch_size: int, lr: float, weight_decay: float,
                 epochs: int) -> None:
        """
        ...
        :param pred_column: Prediction column name or None (Regression).
        ...
        from src_others.tab_ddpm import GaussianMultinomialDiffusion
        ...
        self.is_conditional = (pred_column is not None)

        if self.is_conditional:
            print(f'TAB_DDPM will be used conditioned on the column {pred_column}.')
        ...
        model_params['is_y_cond'] = (pred_column is not None)
    ...
    def train(self, df: DataFrame, random_seed: int = 42) -> None:
        ...
        import zero
        from src_others.tab_ddpm import lib
    ...
    def sample(self, sample_size: int, random_seed: int = 42) -> DataFrame:
        ...
        X_num, X_cat, y_gen = simple_sample(self.diffusion, self.train_dataset, num_samples=sample_size,
                                            ...
                                            T_dict={'cat_encoding': None, 'cat_min_frequency': None,
                                                    'cat_nan_policy': None,
                                                    'normalization': 'quantile', 'num_nan_policy': None,
                                                    'seed': random_seed, 'y_policy': 'default'},
                                            device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"),
                                            seed=random_seed)
...
def _transform_df_to_dataset_for_tab_ddpm(df_train: DataFrame, T: dict, info_json: dict, y: str | None) \
...
def get_synthcity_plugin(plugin_name: str, random_seed: int, **model_args):
    ...
    elif plugin_name.lower() == 'ddpm':
        return TabDDPMPlugin(random_state=random_seed, device=device, is_classification=True, **model_args)
...
def get_synthcity_plugin_hyperparams(plugin_name: str):
    ...
    if plugin_name.lower() == 'survae':
        ...
        return TabDDPMPlugin.hyperparameter_space()
    elif plugin_name.lower() == 'bayesian_network':
