...
from src.evaluation_functions import basic_statistical_measure_num, discriminator_measure_rf, k_means_score, \
    ...
    survival_auc_opt, survival_auc_abs_opt, pmse_index
...
class GenOptimizationConfig:
    ...
    fixed_model_params: Optional[Dict] = None
...
def _validate_gen_optimization_config(cfg: GenOptimizationConfig):
    ...
    pos_metrics = ["basic_statistical_measure_num", "log_transformed_correlation_score", "regularized_support_coverage",
                   ...
                   "survival_auc_abs_opt", "survival_sightedness", "survival", "pmse_index"]
...
def suggest_tab_ddpm_params(trial: optuna.trial.Trial):
    ...
    steps = trial.suggest_categorical('steps', [500, 1000, 2500, 5000, 7500, 10000])

    gaussian_loss_type = 'mse'
    num_timesteps = trial.suggest_categorical('num_timesteps', [100, 250, 500, 750, 1000])
    ...
    params = {
        ...
        'model_params': {'rtdl_params': {'d_layers': d_layers, 'dropout': 0.0}},
    }
    ...
    def objective_function(trial: optuna.trial.Trial):
        ...
        for i, idx in enumerate(folds_idxs):
            ...
            if model_name.upper() == 'CTGAN':
                ...
                if 'fixed_model_params' in cfg and cfg.fixed_model_params is not None:
                    params.update(OmegaConf.to_object(cfg.fixed_model_params))
                print('params', params)
                ...
                if 'predict_column' in cfg:
                    params['pred_column'] = cfg['predict_column']
                else:
                    params['pred_column'] = None
...
def optimize_synthesizer(cfg: MainGenOptimizationConfig):
    ...
    for i, model in enumerate(cfg.optimize_models):
        ...
        # create study
        path = os.path.abspath(os.path.join(PROJECT_ROOT, 'experiments',
        ...
        db_file = os.path.join(path, f'{cfg.experiment_name}_{model}_{cfg.optimization_seed}_{filename[:-5]}.db')
        ...
        study = optuna.create_study(direction='maximize', sampler=sampler, storage=storage, load_if_exists=True,
                                    study_name=f'{cfg.experiment_name}_{model}_{cfg.optimization_seed}_{filename[:-5]}')

        print(f'Number of completed trials before optimization: {len(study.trials)}')
        # start optimization
        study.optimize(objective_function, n_trials=cfg.optimization_trials, show_progress_bar=True, n_jobs=1)
