...
def _train_model(experiment_name: str, model_name: str, config_name: str, train_seed: int, save_filename: str):
    ...
    if model_name.upper() == 'CTGAN':
        ...
        if len(config) == 0:
            print('using default hyperparameters for TAB_DDPM model')
            columns_json = load_columns_json_for_experiment(experiment_name)
            config['model_type'] = 'mlp'
            config['model_params'] = {'rtdl_params': {'d_layers': [256, 256, 256],
                                                      'dropout': 0.0}}
            config['gaussian_loss_type'] = 'mse'
            config['T'] = {
                "cat_encoding": None,
            ...
            config['info_json'] = columns_json
            config['num_timesteps'] = 1000
            config['batch_size'] = 1024
            config['lr'] = 0.002
            config['weight_decay'] = 1e-4
            config['epochs'] = 1000
            config['pred_column'] = None
