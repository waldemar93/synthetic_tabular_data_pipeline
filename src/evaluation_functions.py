...
def _nonparametric_distance(real, syn, n_points):
    """Load the optional SynthCity survival metric only when requested."""
    from synthcity.plugins.core.models.survival_analysis.metrics import nonparametric_distance

    return nonparametric_distance(real, syn, n_points)
...
def survival(real: Tuple[np.ndarray, np.ndarray], syn: Tuple[np.ndarray, np.ndarray], n_points: int = 1000):
    auc_opt, auc_abs_opt, sightedness = _nonparametric_distance(real, syn, n_points)
...
def basic_statistical_measure_num(orig_df: DataFrame, synth_df: DataFrame, num_columns: List[str] = None,
    ...
    orig_med = orig_df[num_columns].median()
    synth_med = synth_df[num_columns].median()
    #valid_med = orig_med != 0  # boolean mask
    #median_dif = abs((orig_med[valid_med] - synth_med[valid_med]) / orig_med[valid_med])
    median_dif = abs((orig_med - synth_med) / orig_med)
    ...
    mean_score = 1.0 - mean_dif.mean()
    std_score = 1.0 - std_dif.mean()
    median_score = 1.0 - median_dif.mean()
...
def regularized_support_coverage(orig_df: DataFrame, synth_df: DataFrame, categorical_cols: List[str],
    ...
    if include_num:
        ...
        for col in num_cols:
            ...
            support_cov = 0
            valid_bucket_count = len(cut_offs) - 1
            ...
            for i in range(len(cut_offs)-1):
                ...
                # Interpolated quantile boundaries can form empty intervals for integer-valued columns.
                # Match the categorical branch by excluding empty real-data buckets from the average.
                if orig_bucket_count == 0:
            ...
            support_cov /= valid_bucket_count
...
def pmse_index(orig_df: DataFrame, synth_df: DataFrame, test_ratio: float = 0.2, alpha=1.2) -> float:
    """
    Propensity Mean Squared Error (PMSE) is used as a metric to evaluate the performance of a Logistic Regression model
    in distinguishing between real and synthetic data. PMSE is a measure that quantifies the discrepancy between the
    model's predicted probabilities and the expected probabilities of a data point being synthetic.

    An adaptation of this score is used
    (see Chundawat, V. S., Tarun, A. K., Mandal, M., Lahoti, M., & Narang, P. (2022). TabSynDex: A Universal Metric for
    Robust Evaluation of Synthetic Tabular Data. arXiv preprint arXiv:2207.05295. for details)

    :param orig_df: The original dataset.
    :param synth_df: The synthetic dataset.
    :param test_ratio: The ratio of test data to split from the combined real and synthetic data. Defaults to 0.2.
    :param alpha: A parameter (base for scaling) used in the pmse_index calculation. Defaults to 1.2.

    :return: The pmse_index score ranging from 0 (worst) to 1 (best).
    """
    ...
    # split real and synth data into train and test, size depends on the provided test_ratio
    orig_df_train, orig_df_test = train_test_split(orig_df, random_state=42, test_size=test_ratio)
    synth_df_train, synth_df_test = train_test_split(synth_df, random_state=42, test_size=test_ratio)
    ...
    # proportion of synthetic datapoints
    c = len(synth_df_test) / len(df_test)
    ...
    # fit the model on the training data
    ml_model.fit(df_train, y_train)
    # predict on the test data
    y_proba = ml_model.predict_proba(df_test)
    # calculate the score
    pmse = sum((y_proba[:, 0] - c) ** 2)/len(y_test)
    e_pmse0 = (len(df_train.columns) * ((1 - c) ** 2) * c) / len(y_test)

    # should be in general close to 1 according to paper, but here close to 0
    ratio = pmse / e_pmse0

    # according to paper this should be -abs(1-ratio), but this somehow ratio is optimal at 0 here, I left it like this
    # for now
    score = math.pow(alpha, -abs(ratio))
...
def k_means_score(orig_df: DataFrame, synth_df: DataFrame, k: int = 10, clip_ratio: float = 1,
    ...
    algorithm to the original data (if parameter cluster_on_real_only is True) or on all data and calculates the
    coverage of the synthetic data for each cluster, where coverage refers to the ratio of the number of synthetic data
    points to the number of original data points in then cluster.
    ...
    :param cluster_on_real_only: Whether k-means is trained only on real data or on the mix of real and synthetic data.
    Defaults to True
