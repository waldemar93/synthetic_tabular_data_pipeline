...
from src.evaluation_functions import (synthetic_eval_check, neg_values_check, basic_statistical_measure_num,
                                      ...
                                      survival_auc_opt, survival_auc_abs_opt, survival, pmse_index)
...
class EvalConfig:
    ...
    metrics: List[Literal["basic_statistical_measure_num", "log_transformed_correlation_score",
                          ...
                          "survival_sightedness", "survival", "pmse_index"]] = MISSING
...
def _validate_eval_config(cfg: EvalConfig):
    ...
    pos_metrics = ["basic_statistical_measure_num", "log_transformed_correlation_score", "regularized_support_coverage",
                   ...
                   "survival_auc_abs_opt", "survival_auc_opt", "survival_sightedness",  "survival", "pmse_index"]
    ...
    if 'synth_preprocess_lambda' in cfg and not isinstance(OmegaConf.to_object(cfg.synth_preprocess_lambda), list):
        raise AttributeError(f'synth_preprocess_lambda needs to be a list.')

    if 'synth_remove_lambda' in cfg and not isinstance(OmegaConf.to_object(cfg.synth_remove_lambda), list):
...
def call_eval_func(experiment_name: str, eval_func_name: str, params: Dict, orig_df_train: DataFrame,
    ...
    if eval_func_name == 'pmse_index':
        return pmse_index(orig_df_train, synth_df_train, **params)
...
def call_visual_eval_func(experiment_name: str, eval_func_name: str, params: Dict, df_orig: DataFrame,
    ...
    eval_folder = os.path.join(PROJECT_ROOT, 'experiments', experiment_name,
                               model_type, 'eval')
...
def evaluate_synthesizer(cfg: MainEvalConfig):
    ...
    _, df_train_training, _ = load_experiment_data(cfg.experiment_name, for_training=True)
    ...
    for model_type in cfg.models:
        for filename in os.listdir(os.path.join(exp_folder_path, model_type, 'models')):
            ...
            for sampling_seed in cfg.sampling_seeds:
                ...
                # if synthetic data already exists, just load it instead
                synthetic_data_path = os.path.join(exp_folder_path, model_type, 'synthetic_data',
                                                   f'{model_name_short}_t{train_seed}_s{sampling_seed}.csv')
                if os.path.exists(synthetic_data_path):
                    ...
                    if ('synth_final_size' not in cfg or cfg.synth_final_size is None
                        ...
                        try:
                            ...
                            # catch only the NaN‐in‐sample error
                            if "found NaNs in sample" in str(e):
                                continue
                            else:
                    ...
                    # error with columns
                    if set(df_synth.columns.values) != set(df_train_training.columns.values):
                        print('ERROR with columns, skip!')
                        continue
                    ...
                    if ('synth_final_size' in cfg and cfg.synth_final_size is not None
                        ...
                        print(f'after removing: {len(df_synth)}, but should be at least: {cfg.synth_final_size}')
                        df_synth = df_synth.sample(n=cfg.synth_final_size, random_state=1)

                    # save csv
                    df_synth.to_csv(synthetic_data_path, index=False)
